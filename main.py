    ...
    def main_menu(self):
        try:
            while True:
                ...
                if choice == "1":
                    ArtworkManagementMenu(self.artwork_dao, self.user_dao, self.user_favorite_artwork_dao, self.artwork_gallery_dao, self.gallery_dao).display_menu()
                elif choice == "2":
                    ArtistManagementMenu(self.artist_dao).display_menu()
                elif choice == "3":
                    GalleryManagementMenu(self.gallery_dao,self.artwork_dao, self.artwork_gallery_dao).display_menu()
                elif choice == "4":
                    UserManagementMenu(self.user_dao, self.artwork_dao, self.user_favorite_artwork_dao).display_menu()
                elif choice == "5":
